...
#main function
def boolean_reduction(variables, minterms):
    
    # minterms are expected to be a list of integers
    bits = len(variables)
    n_var = bits
    a = minterms
        ...
        for i in range(len(unchecked)):
            for j in prime:
                if j == i:
                    s= s+binary_to_letter(unchecked[i])+' + '
        # print(s[:(len(s)-3)])
    expression = s[:(len(s)-3)]

    start_vars = []
    curr_letter = 'a'
    for i in range(len(variables)):
        start_vars.append(chr(ord(curr_letter)+i))

    conv_vars = {}
    for i in range(len(variables)):
        conv_vars[start_vars[i]] = variables[i]

    for l, letter in enumerate(expression):
        if letter in conv_vars.keys():
            expression = expression.replace(letter, conv_vars[letter])

    # need to format expression in terms of user's variables
    return expression
